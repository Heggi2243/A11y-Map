...
if (isDev) {
  console.log('本地開發模式：啟用App Verification繞過');
...
/**
 * 初始化reCAPTCHA
 * 延遲到真正需要時才建立，減少不必要的初始化(提升體感登入速度)
 */
function initRecaptcha() {
  if (recaptchaVerifier) {
    return recaptchaVerifier; // 已存在就直接返回
  }
  ...
  recaptchaVerifier = new firebase.auth.RecaptchaVerifier('recaptcha-container', {
    ...
    'error-callback': (error) => {
      console.error('reCAPTCHA錯誤:', error);
    }
  });
    ...
    if (error.code === 'auth/multi-factor-auth-required') {
      ...
      try {
        ...
        if (code) {
          ...
          console.log('MFA驗證成功:', userCredential.user.email);
          ...
          throw new Error('未輸入驗證碼');
        }
        
      } catch (mfaError) {
        console.error('MFA處理失敗:', mfaError);
